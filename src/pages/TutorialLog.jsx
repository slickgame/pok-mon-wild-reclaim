...
export default function TutorialLogPage() {
  const { data: tutorials = [], isLoading } = useQuery({
    queryKey: ['tutorials'],
    queryFn: () => base44.entities.Tutorial.list('-priority')
  });
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {tutorial.type && (
                  <Badge className="mt-2 text-xs bg-slate-700/50 text-slate-400">
                    {tutorial.type === 'story' ? '📖 Story' : 
                     tutorial.type === 'tip' ? '💡 Tip' : 
                     tutorial.type === 'unlock' ? '🆕 Unlock' : 
                     '📘 Tutorial'}
                  </Badge>
                )}
              </div>
